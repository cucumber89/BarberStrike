...
export interface TdmJoinOptions {
  ...
  /**
   * Respawn-wave cadence, in ms. Dev/test only, gated exactly like `seed`.
   *
   * Most room tests are about something else entirely — damage, movement, the economy — and simply
   * advance several seconds of fake time. With the shipped 12 s wave they now cross a frozen
   * preparation window part-way through and measure the freeze instead of the thing they were
   * written for. Pinning a long wave keeps those tests testing what they say they test; the wave
   * cycle has its own tests that use the real numbers.
   */
  waveMs?: number; prepMs?: number;
}
...
export class TdmRoom extends Room<{ state: MatchState; metadata: { room: string; mode: GameMode; name: string; map: string; bots: number } }> {
  ...
  override onCreate(options: TdmJoinOptions): void {
    ...
    // Tests and tooling may pin the respawn-wave cadence; never in production.
    if (DEV_TOOLS || process.env.NODE_ENV === "test") {
      if (isFiniteNumber(options?.waveMs)) this.waveMs = Math.max(100, options.waveMs);
      if (isFiniteNumber(options?.prepMs)) this.prepMs = Math.max(0, options.prepMs);
    }
  ...
  private addBot(n: number): void {
    if (!this.walk) {
      ...
      this.roamPoints = [...this.map.spawns.map((sp) => ({ x: sp.x, y: sp.y, z: sp.z })), ...this.map.flags.map((f) => ({ x: f.x, y: f.y, z: f.z })), ...this.map.stations.map((st) => ({ x: st.x, y: st.y, z: st.z }))];
    }
    const id = botId(n);
  ...
  private stepBot(p: PlayerState, s: Session, now: number): void {
    ...
    const senses: BotSenses = {
      ...
      enemies,
      los: (ax, ay, az, bx, by, bz) => !this.losBlocked(ax, ay, az, bx, by, bz),
  ...
  private spawn(id: string): void {
    ...
    const sp = pickSpawn(this.map, p.team as Team, {
  ...
  private startMatch(): void {
    ...
    // Two clocks (drop 7): the match runs for `durationMs`, the first LIVE wave for `waveMs`.
    this.state.matchEndsAt = this.now() + MATCH.durationMs;
    this.state.phaseEndsAt = this.now() + this.waveMs;
    this.state.scoreA = 0; this.state.scoreB = 0; this.state.winner = -1;
    this.state.winnerId = ""; this.state.winnerName = "";
    this.projectiles.length = 0; this.fires.length = 0;
  ...
  /**
   * A wave ended: the preparation window opens.
   *
   * Everyone who died comes back HERE, at the start of prep rather than at its end, because the
   * countdown is meant to be spent preparing — seeing where you are, buying, reloading — and none
   * of that is possible from a death screen. Nobody can act until `startWave` releases them.
   */
  private startPrep(): void {
    const now = this.now();
    const endsAt = now + this.prepMs;
    this.state.phase = MatchPhase.Prep;
    this.state.phaseEndsAt = endsAt;
    // A grenade still in the air, or a fire pool still burning, would land on players who cannot
    // move out of the way. The result screen already clears both for the same reason.
    //
    // But a grenade removed in flight was PAID FOR, and near the end of a wave that is not a corner
    // case — it is roughly the last fuse-length of every wave, which an e2e run found by throwing a
    // flashbang that simply vanished. So the ones still in the air go back into the pocket. Fire
    // pools do not: their grenade already went off and did its work, and what is being taken away
    // is an area effect that would burn people who cannot walk out of it.
    for (const pr of this.projectiles) {
      const owner = this.state.players.get(pr.owner);
      if (!owner) continue;
      const w = this.walletOf(owner);
      giveGrenade(w, pr.kind);
      this.writeWallet(owner, w);
    }
    this.projectiles.length = 0;
    this.fires.length = 0;
    this.refillWave();
    this.broadcast(S2C.MatchEvent, { phase: MatchPhase.Prep, winner: -1, endsAt } satisfies MatchEventMessage);
  }

  /**
   * Bring back everyone who is dead and present. Run on every tick of the preparation window, not
   * just on the tick it opens.
   *
   * Because a player who was DISCONNECTED when the window opened is skipped here (a ghost never
   * respawns — see `step`), and `allowReconnection` does not call `onJoin`, so nothing else would
   * ever bring them back: they would sit out the wave they reconnected into as well, dead for up to
   * two full cycles. Running every tick means reconnecting at any point inside the countdown gets
   * you back into it, and it is idempotent — anyone already alive is skipped.
   */
  private refillWave(): void {
    for (const [id, p] of this.state.players) {
      const s = this.sessions.get(id);
      // Survivors are left exactly as they are — health, ammo, armour, position. Surviving a wave
      // has to be worth something, and topping everyone up would make it worth nothing; the player
      // who came through it on 20 health and half a magazine can spend the countdown reloading and
      // buying a plate, which is what the window is for.
      if (!s || !p.connected || p.alive) continue;
      s.respawnAt = 0;
      this.spawn(id); // `spawn` starts the shield at the release while the room is frozen
    }
  }

  /** Preparation over: the next live wave is released. */
  private startWave(): void {
    const now = this.now();
    this.state.phase = MatchPhase.Playing;
    this.state.phaseEndsAt = now + this.waveMs;
    // Domination does not score while frozen (`stepFlags` only runs in Playing), so the tick has to
    // be re-based or `now >= nextDomTickAt` stays true and pays out a burst of catch-up ticks for
    // time nobody could contest.
    if (now >= this.nextDomTickAt) this.nextDomTickAt = now + DOM.tickMs;
    this.broadcast(S2C.MatchEvent, { phase: MatchPhase.Playing, winner: -1, endsAt: this.state.phaseEndsAt } satisfies MatchEventMessage);
  }

  private endMatch(): void {
    // Every kill of the match armed a `respawnAt` that the wave never consumed; leaving them set
    // would hand the next warm-up a pile of already-elapsed timers, so everyone who died would pop
    // back the instant the room returned to Waiting. Overlaps with the warm-up gate in `step` —
    // together they keep the result screen empty, and `Drop7.test.ts` only fails if BOTH go.
  ...
  private updatePhase(): void {
    ...
    switch (next) {
      ...
      // Playing is reached two ways and they are NOT the same thing: from Countdown it is the match
      // starting (scores and wallets reset, everyone spawned), from Prep it is the next wave being
      // released (everything carries over). Dispatching on the target phase alone cannot tell them
      // apart, so the phase we are LEAVING decides.
      case MatchPhase.Playing: if (st.phase === MatchPhase.Prep) this.startWave(); else this.startMatch(); break;
      case MatchPhase.Prep: this.startPrep(); break;
  ...
  private step(): void {
    const now = this.now();
    this.navTick++;
    if (this.state.phase === MatchPhase.Prep) this.refillWave();
    for (const [id, p] of this.state.players) {
      const s = this.sessions.get(id);
      if (!s) continue;

      // Respawn. During a MATCH nobody comes back on their own: the wave brings everyone back
      // together at the start of the next preparation window (`startPrep`). Warm-up keeps the old
      // individual timer, because a warm-up that stops every twelve seconds is just a queue.
      if (!p.alive) {
        // Individual respawn belongs to WARM-UP only. Naming the phases it applies to, rather than
        // the ones it does not, keeps the result screen out of it: `respawnAt` is still armed by
        // every kill during a match and never consumed there, so a "not a wave phase" test let the
        // whole match's worth of them fire at once the moment the scoreboard came up.
        const warmUp = this.state.phase === MatchPhase.Waiting || this.state.phase === MatchPhase.Countdown;
        if (warmUp && s.respawnAt && now >= s.respawnAt && p.connected) { s.respawnAt = 0; this.spawn(id); }
        continue;
      }
  ...
  private detonate(pr: Projectile, now: number): void {
    ...
    if (pr.kind === "flash") {
      for (const [id, q] of this.state.players) {
        ...
        if (strength > 0.05) c.send(S2C.Flashed, { strength, ms: flashMs(strength) } satisfies FlashedEvent);
      }
    }
    if (pr.kind === "molotov") this.fires.push({ x: pr.x, y: pr.y, z: pr.z, until: now + def.effectMs, owner: pr.owner, radius: def.radius });
  }
