...
import {
  ...
  SNAPSHOT_MS, SPAWN_PROTECTION_MS, TICK_MS, WEAPONS, WEAPON_ORDER, duelMapOf,
...
export class TdmRoom extends Room<{ state: MatchState; metadata: { room: string; mode: GameMode; name: string; map: string; bots: number; players: number; slots: number } }> {
  ...
  override onCreate(options: TdmJoinOptions): void {
    ...
    // The mode is read BEFORE the map, because a 1 v 1 chooses only among the arenas built for it
    // (`DUEL_MAP_IDS`, Drop W P1: DOLNA by default, GÓRA on request — `duelMapOf` is the one rule the
    // loading card and the menu apply too). Everything else takes what the lobby asked for.
    this.mode = isGameMode(options?.mode) ? options.mode : "tdm";
    this.state.mode = this.mode;
    const asked = typeof options?.map === "string" ? options.map : undefined;
    this.map = this.duel ? MAPS[duelMapOf(asked)] : (asked ? MAPS[asked] : undefined) ?? MAPS[DEFAULT_MAP_ID];
  ...
  private addBot(n: number): void {
    ...
    // ...and a look each, from the two catalogs walked at different strides so the pairings do not
    // repeat inside a full house. Bots pay nothing for an outfit either: it is paint and trimmings
    // merged into meshes the body already had (`shared/outfits.ts`).
    p.skins = encodeCosmetics({}, BUILDS[(n * 2 + 1) % BUILDS.length].id, OUTFITS[(n * 3 + 2) % OUTFITS.length].id);
