...
export class Game {
  ...
  private syncHud(now: number): void {
    ...
    const me = this.conn.me();
    const rows: ScoreRow[] = [];
    s.players.forEach((p) => rows.push({ id: p.id, name: p.name, team: p.team as Team, kills: p.kills, deaths: p.deaths, score: p.score, ping: p.ping, alive: p.alive, connected: p.connected, assists: p.assists ?? 0, money: p.money, bot: !!p.bot, shaved: !!p.shaved }));
