...
describe("weapon parts: nothing floats", () => {
  ...
  afterAll(() => {
    ...
    reports.sort((a, b) => (order.get(a.id as WeaponId) ?? 99) - (order.get(b.id as WeaponId) ?? 99));
  ...
  async function measure(id: WeaponId): Promise<WeaponReport> {
    if (lib && imported.has(id)) {
      ...
      return judgeWeapon(id, "gltf", parts, receiver!, input);
    }
    const pp = proceduralParts(id);
    const parts = [...pp.parts, ...pp.magazine];
    const receiver = pickReceiver(parts, pp.magazine.map((m) => m.name))!;
