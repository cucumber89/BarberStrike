/**
 * proto-final — DOLNA, the synthesis of the design table (base: proto-cs2 "Wierny CS2", grafts
 * from proto-fidelity "Wierny miejscu").
 *
 ...
 * map, 5.2 m of road and 60 m long, broken OFF-AXIS by the two parked cars (north lane), the van
 * and the two skips (south lane), the two poles and the pine trunks — a 1.7 m strip down the
 * crown of the road still sees all 60 m. Three lanes: WEST (hedge gap ↔ west mesh gap, the pole,
 * the toi-toi and the first skip), MID (wicket/garage ↔ site gate, the van), EAST (gate/drive ↔
 * east mesh gap, the second skip and the car).
 *
 * Changes vs proto-cs2 (the brief's demands): every route passage ≥ 1.5 m (hedge gap, wicket,
 * garage back door, shed doors, hall side door, shell doors, mesh gaps); the shell pulled 4 m
 * towards the street and the front yard shortened so the starts are 35–45 m of path apart while
 * the contested street places stay within 250 ms; the shed has TWO doors (drive side and garden
 * side) as the owner asked; the hall's side door is in the wall towards the thujas (a 1.8 m slot
 * along the west hedge), its roller door towards the house/garden; the house roof is two stepped
 * plates (pyramid silhouette). Grafted from proto-fidelity: the 3–4 tall thujas as a N–S row in
 * the front yard left of the house (Z2), a shrub clump mid-garden west, the site fence corners.
 *
 * +X east (along the street), +Z north (into the plot). Floor top y = 0. Metres.
 * Thin walls straddle a half-metre in z or a quarter-metre in x so the plan tool draws them.
 */
...
add(S(-2.6, 0, 15.5, 0.8, 1.45, 0.9, "leather", "shed_chair_1"));
add(S(-0.6, 0, 15.5, 0.8, 1.45, 0.9, "leather", "shed_chair_2"));
...
const spawns: SpawnPoint[] = [
  ...
  { x: -2.5, y: 0, z: 40, yaw: Math.PI, team: 0 },
...
export const MAP: MapDef = {
  ...
  arenaSpawns: [
    ...
    { x: -6, y: 0, z: 39.5, yaw: 0, team: 0 }, { x: 3, y: 0, z: -12.5, yaw: 0, team: 1 },
