...
- **Prediction**: `LocalPlayer` runs `simulateBody` per frame, stores pending inputs + predicted state; on each snapshot it compares the server state at `ack` and replays if error > 3 cm.
