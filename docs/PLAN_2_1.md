...
### Drop T — The front page, and the 1 v 1 tournament (owner's brief, 2026-09-23)

Four things the owner asked for in one brief: the open matches clickable from the main screen, the
crate given a proper case and a place on that screen, the 1 v 1 map made to play like CS's, and a
**tournament with a bracket**. The tournament is in-room and account-free by the owner's choice
(`turniej`: 4 or 8 entrants, one room, pairs played one at a time to `DUEL.wins`, losers stay and
watch). A pair IS a duel — the freeze, the buy window, the CS economy, the half swap — so the
...
| 2026-09-10 | L (new) | claude/serene-cerf-p40i3w | **Brief for the NIGHT_DISTRICT rework, written for the next agent (Codex) at the owner's request** — *"żeby codex poprawił pierwszą mapę … całą mapę a nie tylko paru miejscach, fajnie lowpoly z klasą"*. `docs/MAP_1_REWORK.md`: the art direction turned into eight checkable rules (silhouette-first, chamfered structural edges, flat shading on a fixed tessellation ladder, dimensions snapped to a 0.05/0.1/0.25 m grid, blocking over texturing, a two-or-three-hue light palette, emissive as the night's subject, fog and sky as deliberate dials) — because the pipeline here is a REALISM one (PBR + generated albedo/normal/roughness at 512 px, world UVs at 1 unit/m) and pointed at a box at night it produces exactly the failed realism the owner named. Plus the reference the brief exists for: a file/line map of `map.ts`'s 19 sections and the seven client render modules; how a solid becomes geometry (toggleable / dressed / plain), how merging by (tag, 12 m zone) — 24 m for detail tags — decides draw calls, and why a new `MaterialTag` costs a mesh per zone; the light cap (`maxSimultaneousLights` 5, range-excluded per mesh, `maxLightsPerMesh` measured 45 → lighting pops); and the trap that `architecture.ts` + `streetscape.ts` are literal district coordinates and solid-NAME regexes, so moving or renaming a wall silently detaches its dressing with only a count > 200 guarding it. MEASURED afresh for the brief rather than quoted: 370 solids / 125 props / 41 lights in 24 distinct colours; per-area density from **2.5** solids per 100 m² (east extension) to **29.2** (storage), with YARD WEST 221 m² carrying 6 solids, 0 props and 0 lights; **22 % of sampled walkable floor has no practical light in range** (83 of 384 samples, in named bands, not scattered); six street spots sit 10 cm INSIDE their own lamp posts, one light is inside the gantry deck, one wall lamp has no light within 3.68 m; both bomb sites hold 4–9 of the 30 items the floor audit allows. Clean and to be kept clean: 0 coplanar pairs, 0 duplicate boxes, 0 degenerate solids, 0 buried solids. No code changed. | docs/MAP_1_REWORK.md | typecheck — test — build — e2e — (docs only) | planned |
| 2026-09-10 | L | claude/serene-cerf-p40i3w | Second brief from the owner — the map is to be REBUILT, not only repainted, and it has to survive **6v6 tournaments next week with a real prize** (a free haircut). He named defects by symptom; symptoms are not coordinates, so each was measured against the shared map data and the real walk grid and written into `docs/MAP_1_REWORK.md` Part 2. **Stairs into a wall**: `east_stair_4`/`east_stair_5` interpenetrate `cw_machine_a` with faces at exactly 0 mm over 2.1 and 1.68 m² — the wall and the shimmer are one defect. **Stairs to nowhere**: `shed_roof` (35 standable cells, **0 reachable** — `lookout_rail_s` seals the 2 cm step off `yard_lookout`) and `container_top` (60 cells, 0 reachable, where the map's own comment puts the intended perch on the 2.5 m stack below); every other deck is 100 % reachable. **Lamps**: eighteen props in three variants that do not behave alike — six stand on a real collision post, ten identical-looking ones are drawn entirely by `props.ts` and have NO collision, so the same lamp is hard cover on Main Street and thin air in the yard; measured and clean, so not the cause: every floor-standing prop does sit on its floor. **Textures inside each other**: **54** pairs of solids share a coplanar face at exactly 0.0 mm while interpenetrating, 44 of them over 1 m² — `floorAudit.test.ts` checks TOP faces only, which is why none was ever caught, and 20 of the 54 are one mistake repeated in `districtExpansion`'s `room()` corners and the car-wash block. **Tournament fairness**: walked median path from each team's spawns puts TAPER **10.7 m** closer to bomb A and **17.4 m** closer to bomb B — both sites on the same side, no compensating pair — while FADE owns two of three buy stations; and each team's eight spawns span **55 m of x inside 3 m of z**, so a six-stack starts strung across the whole map and cannot leave spawn as a unit. Kept: 0/16 spawns see the map centre. No code changed. | docs/MAP_1_REWORK.md (Part 2, §13-16) | typecheck — test — build — e2e — (docs only) | planned |
| 2026-09-10 | L | claude/serene-cerf-p40i3w (merged `codex/map-pass`) | **Reviewed and merged the Codex art pass, then fixed what it could not see.** Codex worked Part 1 of `docs/MAP_1_REWORK.md` and did it properly: `DISTRICT_LIGHTS` — a named three-hue palette in the shared data, 41 lights in 24 colours → **53 in 5**, and unlit walkable floor from **22 % to 0 %** (1 of 425 sampled points); practicals moved out of the posts and decks they were buried in, gated by a new `districtLighting.test.ts`; flat colour blocking with the district's normal maps dropped; 5 cm chamfers; a rooftop silhouette for each of the thirteen areas, anchored to named solids so a moved building carries its landmark and a missing anchor THROWS; Polish wayfinding on every visible sign; `selectPracticals` replacing the first-in-list light cap; and a portable `map-review.mjs` (honours `PW_CHROMIUM`, 11 extra area views, real postfx presets). Its own `district-audit.ts` SHA-hashes the collision boxes, spawns, stations and flags — 370 solids before and after, so none of it touched what the server ticks. **Then the fixes.** (1) The chamfer bevelled every face, including the 312 of 912 that are pressed flat against a neighbour (240 at joints between solids, 72 at tile cuts) — a 10 cm groove across every wall that changes box, five per 62 m facade; `chamferData` now takes a six-bit face mask and `wallFaces.ts` derives it from box arithmetic. (2) **54 pairs of solids shared a face plane at exactly 0.0 mm**, 44 over 1 m², invisible to `floorAudit` because it gated TOP faces only; extended to all six directions, gated on both maps, and all 54 fixed at source — twenty of them one mistake repeated in `districtExpansion`'s `room()` corners, the car wash's four and GÓRA's five. (3) **`east_stair` ran through the car wash**: from the gantry at x 30..32.4 it crossed `cw_s_b` and clipped `cw_e_a`, so the walk grid could stand on the bottom three steps and the top three and nothing between; moved into the east lane at x 32.4..34.8 with the landing widened to meet it. (4) **Two routes to nowhere**: `shed_roof`, 10.5 m² two centimetres below the lookout and sealed by a full-width rail (now 0 → 39 of 40 cells reachable); and the docblock's "climbable container stack", dead on its second crate because the cell straddling a 0.6 m joint needs ground within one step height (0.4 m) — 1.0 m makes each crate a step and the chain reaches the roof. Also moved the wash machine that stood inside the staircase and the planter with a climbing crate buried in it. | `apps/client/e2e/tools/map-audit.ts` (seven checks, re-runnable, no GPU); `apps/client/e2e/out/map1/audit-after.md` | typecheck ✓ test ✓ (847: shared 288, skins 2, server 178, client 379) build ✓ check:weapons ✓ 19/19 e2e — | in progress |
| 2026-09-10 | L | claude/serene-cerf-p40i3w | **Tournament pass, on the owner's decision: keep the map asymmetric and force the sides to change.** First a CORRECTION to this session's own earlier measurement — "TAPER is 10.7 m closer to site A" compared `map.spawns` by their `team` field, and in Bomb that is not who stands there: `TdmRoom` picks the set from the ROLE (attackers take the team-0 points, defenders the team-1 points, every round), and `bombAttackTeam` gives each team six rounds of each, so both teams already play both sides and the asymmetry cancels. `map-audit.ts` §7 now reports attack-vs-defence for Bomb and team-vs-team only for the modes that never swap. That rule was one expression inside a 1 500-line room file with nothing guarding it; it is now `bombSpawnSide()` in shared with three tests (each team half the rounds on each set; attacker always south, defender always north; never both on one set). **Spawns**: each side's eight points spanned **55 m of x inside 3 m of z**, so a six-player side started in six different lanes and could not leave spawn as a unit. Two rows of four, 6 m apart, offset so the closest pair is 3.4 m → spread **21.0 m**, both sets identical, still 0/8 seeing the map centre. Centring them also flattened the sites against each other: the defender's head start was 10.7 m to A against 22.3 m to B (B harder by 11.6 m), now **16.2 against 19.7** — a 3.5 m difference. **Lamps**: ten of eighteen were drawn by `props.ts` with no collision at all while six identical ones stood on a real post, so the same lamp was cover in one street and air in the next; every standing lamp now has the post, and the two practicals that ended up inside a new post moved to the head (`districtLighting.test.ts` gates it). The lone tyre in the open yard became three against the wash. **Two harness fixes, one fragility**: staging that depended on where spawn points happened to be — `faceOff` took the first clear pair over 3 m (a 3.4 m duel instead of 12 m) and now takes the farthest inside the cap; `clip()` swung from wherever the respawn dropped Bravo, and a defender spawns with his back to the north wall, so the attacker was placed inside it. | `apps/client/e2e/out/map1/audit-final.md` | typecheck ✓ test ✓ (850: shared 291, skins 2, server 178, client 379) build ✓ check:weapons ✓ 19/19 e2e — | review |
| 2026-09-10 | L | claude/serene-cerf-p40i3w | **The map, RENDERED for the first time — and `/viewer`.** The owner said the map still looked bad; he was right, and every judgement I had made about it until now came from data. Rendered, the cause was one thing and it was not geometry: the scene drew straight to the display with **tone mapping off**, so every value over 1 clipped to flat white — at night that is most of what a light touches. The yard concrete came out as near-white paper, every practical was a white blob with no fixture inside it, and anything no lamp reached was pure black. No repaint could have fixed that. ACES + `BASE_EXPOSURE` 1.85 (up from 1.25 to pay for the curve), hemispheric ambient **0.48 → 0.30** (at 0.48 the sky lit every wall as brightly as a lamp did, which made Codex's 53 lights decoration), and the district's sky, which ramped to `#53636a` — a bright overcast afternoon — now ends in the city's own glow on a dark horizon. Also found by looking: the **lavender wedge under the horizon** in every high shot was Babylon's default clear colour, because the sky dome is a 0.55 slice and nothing clears what is below its rim. `map-review.html` now grades exactly as the game does, or the review is a picture of a game nobody plays. **`/viewer`**: a spectator joins with `spectator: true` and the room builds no `PlayerState` — no schema field, no seat, no scoreboard row, nothing it can send that moves a round; four negative tests say so. `maxClients` stopped being the player cap (Colyseus gates every client with one number) and that exposed a real leak: `/health` and `/rooms` counted SOCKETS, so watchers would have made a room look full to somebody who wanted to play — the room now publishes the count only it can tell apart. Client is its own page rather than a mode of `Game.ts`, reusing `buildMap` and `RemotePlayer` at the players' own interpolated instant; the camera's maths is a Babylon-free module with 7 tests (forward follows the look, up stays world-vertical, diagonals normalised, a slept tab clamped). Twelve viewpoints + both sites on number keys, follow-a-player, live fps/draw-call/position. Verified in a browser: 14 jump targets, camera really moves (street 373 draw calls → yard 62), 0 page errors, **`/health` reports 0 players while a watcher is connected**. | `apps/client/e2e/out/map1/look/` (before) · `.../after2/` (after) · `.../viewer/` · `apps/client/e2e/tools/viewer-check.mjs` | typecheck ✓ test ✓ (863: shared 291, skins 2, server 183, client 387) build ✓ e2e — | review |
| 2026-09-13 | G | claude/serene-pascal-hrwuf2 (harness-assigned) | **GÓRA rebuilt as the 1v1 tournament roof — GÓRA (DACH).** The flat is gone; the map is the roof of the same block: a 34 × 22 m fenced deck, the lift machine room as a 2.0 m PERCH with a 1.35 m lip on the court sides and open stair ends, a stair head in two corners hiding the starts behind a gate wall and a wing wall, a court with chairs + mirror + neon on each long side, a yard with a vent tank on each short side, each long edge cut into two covered lane halves by a 3.9 m fan house, a 4.4 m cage all round, the estate beyond the wire. **Fairness is a construction**: everything off the centre line is placed with its 180° twin (`pair()`), so both starts are identical to the metre and both players play the same hand; `gora.test.ts` checks the symmetry on the output and `map-duel.ts` walks twelve places from both starts with the real mover — **0 ms** difference on every one (brief: < 250). No team-0 point sees any team-1 point standing or crouching; the duel start is visible from 152 of 1 940 surfaces, none over 9 m. Median clear line 7.6 m, p99 20.7, 1.6 % over 20 m; the perch sees 30 % of the ground. Cover speaks one language (0.8 / 1.3 / ≥ 2.0 / 2.8–3.9) and every real jump — standing + mantle beside a top, the 4.4 m sprint parabola across — is chained from the deck by the test: highest standable top 2.2 m (the lockers, by a hop off the perch), cage 2.2 m above it; a lower cage FAILED that test (crate → vent unit → wire) and so did a crate in the yard (crate → unit → pallets → tank → stair head), both fixed by geometry. Chairs carry a two-box collision proxy the size of the drawn chair; bins are `bin` solids, not `trash` props. **Mode `duel`**: two seats, at most one bot and only if asked, 4 s freeze with 6 000 to buy, one life, 60 s, kill / trade / clock-on-health, sides swap every 3 rounds, first to 6, a 15-min cap that only ends a match somebody leads, the Ended → Waiting loop as the rematch, a walkover when the other player is gone for good — same Prep/Playing machine, `bomb.round`, `scoreA/B`: no schema or protocol change. Client: duel HUD line, mode art, bots slider capped at 1 in a duel, `/viewer` spots per map, `map-review.html?map=`. Rendered on SwiftShader (74–310 draw calls, 27 k verts) and played live against a bot (rounds turn over, the bot crosses the roof and wins rounds, `/health` counts 2). | `apps/client/e2e/out/g/{duel.md,rotation.md,audit.md,plan.txt,shots/*.png}`; docs/MAP_2.md (§ Rebuilt as the 1v1 roof); regenerate with `map-duel.ts`, `map-rotation.ts`, `map-audit.ts gora`, `map-plan.ts`, `gora-shots.mjs` | typecheck ✓ test ✓ (shared 305, skins 2, server 190, client 396 — three character view tests failed under the load of the full run beside a build and pass alone, one of them also failed on the untouched baseline) build ✓ check:weapons ✓ (22/22) e2e — | review |
| 2026-09-14 | P (polish, owner's brief) | claude/practical-dirac-airniy (harness-assigned) | **Gameplay-feel pass: weapons, procedural character, rendering, measurement.** Found first (new `feel-cycle.mjs`: two real clients, idle → walk → sprint → ADS → fire → reload → switch → grenade → death → respawn, both views + per-step frame stats): the sprinting gun left the frame; reload audio and animation had drifted 200–400 ms apart (rifle seat 0.72–0.82 vs a thud at 0.60, six shotgun shells vs four sounds); the reload ignored the magazine; ADS over a reload; pose blends per frame and a mouse sway ∝ dt² (4.8× between 30 and 144 fps); third-person hands 0.17–0.24 m off the fore-end; no environment for any metal. **Done**: `combat/reloadTimeline.ts` owns the choreography and `reloadCues()` reads the audio beats off the same frames; the reload carries `shells`/`empty` (tactical reload skips the charge, N shells load N — VZ-9 pinned by test); reload keeps the gun up and the sights down, R during a draw is held not dropped; sprint pose in frame; `1−exp(−k dt)` blends and a substepped sway (test: same pose at 30/60/144 Hz); in ADS the view carries the recoil and the sight stays on the crosshair (vm-fit mean ADS offset 0.36 → 0.23 px); kick from `recoilUp`. Character: two-bone arm IK (hands on grip and fore-end, every gun / pitch / build, no allocations), bladed long-gun stance with the bore still on the aim (hand-pose 3.5–3.9°), sliced vest-clearance oracle, feet planted under a turning torso with a step past 0.7 rad, strafe scissor, backpedal stride, rise/fall from replicated `vy`. Rendering: `nightEnv.ts` 16 px night cube + hand-built spherical polynomial on weapon / hand / kit materials only, and a two-light first-person rig on first-person meshes only. Measured: `cycle-leaks.mjs` 10 × menu → match → armoury → menu stable (114 MB / 46 MB, 44 engine textures every cycle); `profile.mjs` medium 165–350 draw calls; `reconnect.mjs` crashes on the untouched sources too; e2e 16/18 with the same 2 failures before and after (Deferred). All render figures are SwiftShader. | docs/GAMEPLAY_POLISH.md; apps/client/e2e/out/feel/ (final), stage4_before_after.png, stage4_zoom.png, weapons-before/ & weapons-after/, weapons/vm-fit.md, perf/cycle-leaks.json; scratch baseline copies not committed | typecheck ✓ test ✓ (shared 305, skins 11, server 190, client 424 — one outfit test timed out under the full run beside the browser tools and passes alone) build ✓ check:weapons ✓ (22/22) e2e 16/18 (both failures pre-existing) | review |
| 2026-09-22 | S (seats, owner's brief) | claude/practical-cerf-dwv2a1 (harness-assigned) | **Deathmatch became an open lobby: eight bots AND a crowd.** The owner asked for a deathmatch that takes eight bots plus as many players as want in; it took four of them, because a bot spent a human seat out of `MAX_PLAYERS` (12 − 8 = four people). Capacity is now the MODE's: `OPEN_MODES` = {tdm, ffa} seats its bots ON TOP of `OPEN_MAX_PLAYERS` humans (32 by default, `FB_MAX_PLAYERS` moves it 2..64, clamped not refused — an empty env var is unset, not zero); every other mode keeps twelve bodies with a seat per bot, and a duel keeps its two. One shared table (`modeCapacity`) is read by the room, `/health` (`openMaxPlayers` beside `maxPlayers`) and the tests, so the server that enforces it and the screen that prints it cannot disagree. Fixed on the way: `metadata.slots` was the HUMAN seats while `metadata.players` counted bots, so a twelve-body room with two bots already published itself as 12 / 10 — it is bodies now (`roomSize()`). **Measured, not assumed**: live, 16 SDK clients + 8 bots = 24 bodies in one room (the 13th would have been refused before), then 32 + 8 = 40 with the 33rd refused "room full", tick mean 0.43 ms of 16.7; in the suite 40 bodies cost 0.32 ms/tick mean (the 12-body room costs 0.25 — the bots' planning is the cost, not the crowd) and 3.6 kB/s per client against 3.3 at ten bodies. Client: the scoreboard scrolls with a sticky head and sorts team rows by kills (arrival order is unreadable at 20 a side), and the lobby says under the bot slider whether a bot costs a seat in this mode. No schema field, no tick or snapshot change (L6), nothing gated (L1). | apps/client/e2e/out/seats/crowd.md (regenerate: `node apps/client/e2e/tools/crowd-check.mjs`, `N=34`); docs/HOSTING.md (§ How many people fit) | typecheck ✓ test ✓ (954: shared 309, skins 11, server 197, client 437) build ✓ e2e — (not run) | review |
| 2026-09-22 | D (duel + shop + settings, owner's brief) | claude/practical-cerf-dwv2a1 (harness-assigned) | **The 1 v 1 became Counter-Strike's, the buy menu became CS2's, the settings got tabs — and four people played it.** Owner: "zwiększ czas kupna w 1 v 1, zrób tryb dokładnie jak w CS:GO, sklep jak w CS2, tak samo ustawienia, proste i czytelne, zrób różne osobowości które grają w to i testują". **Duel**: freeze 4 s → **15 s** (`mp_freezetime`) with the shop open **5 s past the release** (`buyTailMs`, so 15 + 5 = CS's `mp_buytime` of 20 — a new `windowEndsAt` on `BuyContext` counts it down without touching `releaseAt`, which perks need); the flat 6 000 a round is gone for CS's carried wallet — $800 opens a half, $3,250 a won round, the $1,400→$3,400 ladder a lost one, a draw pays the first rung and climbs nobody's streak, kills pay by weapon (clippers $1,500 / shotgun $900 / SMG $600 / sniper $100 / rest $300), $16k cap, both wallets reset at the swap; **what you carry you keep if you survived**, a casualty loses it all. The owner's one departure: a **$2,500 floor** on every round but a half's first (buys a carbine, or an SMG and the heavy plate — deliberately not the rifle). No schema field: the purse and the loss streak are server-side, money was already replicated. **Shop**: rows → CS2 **tiles** (the whole tile is the button, price under the picture, state as a chip) and four slot-aisles → CS2's **five** (pistols / mid-tier / rifles / gear / grenades), which is also what fixed a pistol round that opened on an aisle where $800 bought nothing; a dead aisle now names one with something in it. `modeAllowsItem` is one shared rule for what a mode stocks, read by the menu AND the server — Bomb's launcher/perk gate was a server special case, the duel had none at all. **Settings**: one 40-control scroll in English → six tabs in Polish with a hint line under anything that needed one. **Four personas played it** (first-timer, CS2 veteran, 48-year-old on a scaled laptop, somebody with 15 minutes) and every finding that was in scope was fixed the same session — including two real bugs: the duel NEVER showed a round-end card (`RoundBreak` read `bomb.result`, mirrored only in Bomb) and the HUD advertised "B: SKLEP" during the 3 s break when the shop is shut. Plus a lying tool: `settings-shots.mjs` measured a container that never scrolls and reported "fits" while a tab was clipped. | apps/client/e2e/out/duel/duel-check.md (live over real sockets: freeze 15.0 s, tail buys at +1.5 s and refuses at +6.5 s, floor and kept gun observed); apps/client/e2e/out/ui/shop-duel-*.png (`ui-fit.mjs --mode duel`, 8/8 viewports PASS); apps/client/e2e/out/settings/ (`settings-shots.mjs`) | typecheck ✓ test ✓ (970: shared 315, skins 11, server 206, client 438) build ✓ e2e — (specs updated for the five aisles, not run: needs servers) | review |
| 2026-09-23 | R (release pass, owner's brief) | claude/practical-cerf-dwv2a1 (harness-assigned) | **"Zrób w jak CSGO i popraw całą grę bo ja wypuszczam" — the CS rules generalised, and the suite green for the first time.** **One CS block** (`packages/shared/src/cs.ts`): the freeze (15 s), the buy window past it (5 s = `mp_buytime` from the round's start), the money ($800 / $3,250 / the $1,400→$3,400 ladder / $16k cap) and the kill table (clippers $1,500, shotgun $900, SMG $600, sniper $100, rest $300). **Bomb was the mode that IS CS and was drifting from it**: it froze for 30 s and shut the shop the instant the round went live, and paid a flat $300 a kill while the duel paid by weapon — so the two modes taught different games. Bomb now reads the same block, plus CS's planted-bomb bonus ($800 to an attack that planted and still lost). The 1 v 1's own numbers became references to it; only the owner's $2,500 floor stays its own. **E2E 18/18, first time in this ledger** (was 16/18, then 17/18): the crate reel stopped at a viewport-derived position (`min(46vw,525px) - 4237.5px`) that left the prize **466 px** from the marker, and announced the prize at 3 s against a 4.2 s roll — the stop is now the cards' own geometry (`--win-index`, cell + gap; measured after: prize centre and window centre both 640 px) and the reveal waits for `animationend`. **The open lobby's own bug, found and fixed**: eight spawn points a side against 32 seats put people INSIDE each other — measured 10 overlapping pairs at 24 players, 0 with `spreadSpawn` (a golden-angle ring validated against the collision world and the living). **And its other one**: TDM's 40-kill limit is a TEAM total, so a 40-body room ended in about a minute — `scoreLimitFor` scales it with the roster (40 at twelve, 150 ceiling), computed from the replicated player map by the server that ends the match AND the HUD that prints it, so no field was added to the wire. | apps/client/e2e/out/duel/cs-round-check.md (live: Bomb's freeze 15.1 s, tail buys at +1.5 s and refuses at +6.5 s, perks and launcher refused, "BOMB DETONATED"); e2e 18/18 in 10.3 min | typecheck ✓ test ✓ (977: shared 321, skins 11, server 207, client 438) build ✓ **e2e 18/18 ✓** | review |
| 2026-09-23 | R2 (release: version + reconnect, owner's brief) | claude/practical-cerf-dwv2a1 (harness-assigned) | **v2.1, and the reconnect "harness crash" turned out to be the bug.** The owner set the release number: `GAME_VERSION` "2.0 beta" → **"2.1"** and all five package.json files → 2.1.0 (live `/health` confirms `"version":"2.1"`). Then he asked for `reconnect.mjs` to be checked — deferred twice as "a harness or an older regression". It was neither: **the tool was crashing ON a real bug and the stack trace was hiding it.** The Colyseus SDK 0.18 reconnects a dropped socket itself (15 attempts, backoff) and reports every failed ATTEMPT through `room.onError` with no code and no message; `Game` mapped any error to "connection lost", so MEASURED on a 1.5 s outage: `onError undefined undefined` at 0.7 s, **player back in the MENU at 1.0 s**, and the SDK's own chain reporting "✅ reconnection successful" at 3.7 s — into a room the client had thrown away, with the server still holding the seat until its grace expired. Fix: `errorEndsMatch` (an error while the SDK is retrying is not a leave; it raises the HUD's reconnecting flag instead) and `room.onReconnect` clearing it; our own retry loop stays for the one case the SDK refuses, a room younger than its 5 s minimum uptime. After: **same session back, still alive, inputs acked 39 → 48**, and A sees BRAVO reconnect. Two stale things in the tool fixed with it — it read the ack off the replicated player (it became a per-owner message in the performance pass) and never pressed ENTER MATCH, so "can B still play" was measuring a player who had never spawned. Also: the smoke spec's 10 s poll is now 25 s, after it read 0.44 on the third full suite run of the session and passed alone at 11.6 s — same assertion, room for a slow renderer; and the body-build spec stopped comparing two bodies at ONE instant — each breathes on a phase seeded from its own player id, so "crowns 1.877 vs 1.853" was 24 mm of SWAY failing a 20 mm tolerance about one run in four. It now takes nine readings over a full breath per body and compares the means, which is the question it meant to ask (the bodies, not the moment). | apps/client/e2e/out/duel/cs-round-check.md (§ Reconnect); `apps/client/e2e/tools/reconnect.mjs` | typecheck ✓ test ✓ (978: shared 321, skins 11, server 207, client 439) build ✓ e2e 18/18 ✓ | review |
| 2026-09-23 | P (perks, grenades, launcher — owner's brief) | claude/practical-cerf-dwv2a1 (harness-assigned) | **"Ulepsz perki, granaty wszystkie, i granatnik" — four grenades were lying to the player and the server agreed with them.** Thirty agents mapped, designed and then attacked the upgrade; what survived is below, and what the attack killed is in Deferred. **Server, four bugs, one test each**: a thrown **knife passed through a player standing against a wall** — `stepProjectile` sets `stuck` inside the contact that carries the knife INTO the wall, on the same tick a body against that wall is swept, and the sweep was guarded by `!pr.stuck`; at 28 m/s a tick is 0.47 m of dead zone. A **molotov burned in mid-air**: it goes off on contact with a body or a wall (`impactOnPlayer`) and the pool was left at the point of impact, while the burn band is only `f.y − 1.0 .. f.y + 1.6` — MEASURED, thrown flat from the 1.62 m eye at 15 m/s the bottle is still at 1.50 m two metres out, so a direct hit at close range did **nothing at all**; the impact point now drops to the floor under it before the Boom goes out, so the fire the client draws is the fire the server burns with. The **damage wedge pointed at the thrower**, not at the blast — an arrow over your shoulder while the fire is at your feet — so `applyDamage` takes the source point and blasts and fires pass it. And a **flashbang blinded straight through a smoke cloud**: smoke stops light, and the HE blast deliberately still goes through it, because it does not stop shrapnel. **Balance**: the launcher's shell **95 → 110**. `balance.test.ts` has had a rule since drop B — anything in [95, 100) is an accident, perfect play and the victim lives — and the launcher was the one weapon it skipped, because its damage lives in `GRENADES` and not in `WeaponDef`: the most expensive gun in the shop, one round in the tube and 2.6 s to reload, landed a perfect direct hit and left a full-health player standing on **5 HP**. The rule now covers explosives. **Client**: `sfx.bounce` has existed since drop 2 with **zero call sites** — a grenade skittering past your feet made no sound — so the view, which owns the re-simulation, emits a client-local `grenadeBounce` (no wire field: bounces are deterministic) and the audio module voices it, capped at two a frame; the **victim of a flash heard the same crack twice** (positioned from `boom`, then centred on their head from `flashed`) and now gets the ringing alone; the smoke's hiss was capped at 6 s against a 12 s cloud. The **cook ring belonged to every grenade**, so a smoke took the crosshair away for the 180 ms of its wind-up and drew an empty circle where the sight had been — and it measured the FUSE, stopping at 0.86, so the arm swung by itself at a point the ring had never marked; it is the frag's alone now and reads **1.000** at the release. One shake number covered the frag and the molotov and none covered the shell: each blast now has its own amplitude over its own radius. **Perks**: the steroids' two-second gate is on the row (`CZEKA 2s` / `+6/s`) — a player who bought the perk, traded shots and watched their health sit still had no way to tell a working perk from a broken one; a perk's last five seconds are marked; the flask's haze thins over its last four; and the **fade**, which is spent at the instant you respawn and therefore never appears in the perk list, finally shows the shield it bought. **Shop**: the launcher card advertised **no damage at all** (`WEAPONS.launcher.damage` is 0 — the damage is the shell's) under a `Zasięg 40 m` that means nothing for a projectile, and every perk card said one thing, `Czas` — how long, never for what. **P3, the last English in the game**: four perk blurbs, two plate blurbs, `ARMED`, `BROKEN`. And one shared rule, `perkTimed`, for "counting down or armed open-endedly", with slack for the two clocks — the HUD and the shop decided it separately and both could flip on the frame a perk was bought. | apps/client/e2e/out/nades/grenade-feel.md (regenerate: `PW_CHROMIUM=/opt/pw-browsers/chromium node apps/client/e2e/tools/grenade-feel.mjs` with the dev servers up) — live on the shipped client: 2 bounces and **2 voices** where there were always 0; shake frag 0.016 / molotov 0.023 / shell blast 0.049 beside the launcher's own 0.014 recoil; smoke in hand crosshair=1 ring=0, frag crosshair=0 ring=1 at 1.000; `STERYDY · CZEKA 2s` → `+6/s`; the cards | typecheck ✓ test ✓ (987: shared 325, skins 11, server 210, client 441) build ✓ e2e 18/18 ✓ ui-fit 8/8 ✓ | review |
| 2026-09-23 | T (front page + tournament, owner's brief) | claude/practical-cerf-dwv2a1 (harness-assigned) | **"Skrzynka na main ekranie, mecze klikalne, turniej 1 v 1 z drabinką, mapa jak z CS:GO" — three of the four shipped and the fourth measured into a decision.** **FRONT PAGE**: the title screen has listed open matches since the 2.6 menu as `div.srv-row.static` — you could SEE a game running with five people in it and still had to press GRAJ, scroll a lobby column and find the same row again to join. They are buttons now (six, two columns of three, one line each), with the nickname field and SZYBKA GRA in the panel's footer, so the whole path from "somebody is playing" to being in it is one screen. The crate moved out from behind SZAFA → SKRZYNKI onto the same shelf: `Crates` grew a `compact` variant — the counter, the case and the SAME reel, because the reel's stop position is measured to the pixel by `armoury.spec.ts` and a second implementation of it would be the first thing to drift — and the case itself got brass banding, rivets, a sheen on hover and a slow breath while there is something in it. Measured with `menu-fit.mjs`, 5/5 PASS; the strip's height had to be made REAL (`min-height: 0` + clipped overflow, without which the panels size to their rows: measured 189 px of content in a 176 px strip, over a page footer at 621) and the responsive caps moved onto it, with a new `max-height: 640px` rule for browser zoom, which arrives as 1024×576. **MAP**: GÓRA has been the duel arena since it was built and *nothing pinned the mode to it* — a 1 v 1 ran on whatever map the room carried, by default the 100 m night district. `DUEL_MAP_ID` is now read before the map is chosen. The start did NOT move: the audit says the duel opens 29.7 m apart with 19.6 m of walking to the perch, so four shorter starts were built and measured — all four keep the sides at 0 ms and hide the start as well or better (5.0–6.9 % of surfaces against 7.8 %), and **all four end the walked route onto the perch STUCK**, because from the south strip the mover reaches the stair side-on. A 25 % shorter approach is not worth breaking the route to the map's contested middle. **TOURNAMENT**: a new mode `turniej`. The bracket is pure and tested without a server (`tournament.ts`, 18 tests: the draw, byes, walkovers, the wire format, a nickname that tries to break it); the room is the duel's machine with two additions — `playingNow`, which is why six waiting bodies do not make every round a draw, and `finishPair`, which sends somebody up the bracket instead of ending the evening. Everything else — freeze, buy tail, CS economy, half swap, first to 6 — is the duel's, unchanged. Leaving is handled where it happens: in the pair being played it is a walkover, anywhere else the entrant is struck out so their opponent goes through rather than waiting for somebody who is not coming back. **One new replicated field** (`bracket`), which L6 makes a stop-and-ask: the owner chose it over riding the unused `BombState` strings and over broadcasting on change. The HUD draws the bracket between pairs and on the result screen, one line while a pair is on, and tells a dead player which of the three things they are — playing, waiting for their pair, or out — because a tournament leaves six of eight people dead for minutes and the card was counting down a respawn that was never coming. | apps/client/e2e/out/menu/ (`menu-fit.mjs`, 5/5 PASS); apps/client/e2e/out/g/duel.md (`map-duel.ts`: 0 ms, none of the 36 start pairs visible); apps/client/e2e/out/turniej/ (`tournament.mjs` — a four-player bracket played out live: PÓŁFINAŁ RYSIEK 3:6 ZDZICHU decided, TY:MIREK up next, FINAŁ ZDZICHU vs —) | typecheck ✓ test ✓ (1017: shared 344, skins 11, server 217, client 445) build ✓ e2e 19/19 ✓ (the new front-page spec among them) | review |
| 2026-09-23 | T (tournament, polish from the live run) | claude/practical-cerf-dwv2a1 (harness-assigned) | **What playing a whole bracket showed, fixed the same day.** The live tool (`tournament.mjs`, one human and three bots) played a draw of four to its end — TY 0:6 RYSIEK, MIREK 4:6 ZDZICHU, final RYSIEK 4:6 **ZDZICHU** — and the screens it photographed carried three things the tests could not: the bracket card **floated over the result screen and covered the word PORAŻKA**, so the whole draw is now a THIRD TAB of the result card (DRABINKA, beside PODSUMOWANIE and TABELA) and the floating card is for the break between pairs only; the top bar read **FADE 3 — 6 TAPER** in a pair of eight people, so `pairNames` takes the two off the bracket and the bar carries nicknames, in side order; and the result sentence said **"Pierwsza drużyna do 6 punktów"** about a mode with no teams and no limit — a tournament is decided by its final, and says so. The fit harness grew a finished eight-player tournament (`case=turniej`) so the bracket tab is measured at both sizes like every other screen. Also from the run: the tool read `mapId` off the HUD, which does not carry it, and sampled the room in the beat between `startMatch` drawing the bracket and `beginDuelRound` spawning the pair — reporting "one player on the map" for a mode whose point is that there are two. | apps/client/e2e/out/turniej/turniej.md (the full draw, a champion); apps/client/e2e/out/turniej/03-koniec.png; apps/client/e2e/out/ui/result-turniej-*.png (`ui-fit.mjs`, PASS at both sizes) | typecheck ✓ test ✓ (1023: shared 344, skins 11, server 217, client 451) build ✓ **e2e 19/19 ✓** ui-fit ✓ | review |
| 2026-09-25 | U (UI jak w CS/CoD, owner's brief) | claude/practical-bardeen-07gu1m (harness-assigned) | **"Przebudowa UI w grze: prościej, czytelnie, jak CS/CoD — też przejścia i przerwy", built by the ULTRON fleet (docs/ULTRON.md).** Tool first: `hud-states.mjs` photographs the real HUD (28 → 69 scenarios × 3 sizes, words / blocks / min font / zones / overlaps / apart / anims) and `hud-diff.mjs` diffs galleries pixel by pixel; the before run was 1535 words over 28 moments, 298 text nodes under 12 px, 9 px minimum, the match clock shown during a 15 s freeze. Wave 1: 9 recon scouts + a visual critic, 3 rival designs (CS2 / CoD / minimalist) scored by 3 judges (CS2 won 122.5 / 116.5 / 106), synthesis attacked by a completeness critic → `docs/UI_U_SPEC.md`. Wave 2: P0 split `Hud.tsx` 640 → 120 lines into zone components and moved the CSS per zone with 0.000 % pixel change after every commit; P-SRV: bomb break 5 → 7 s, a real 15 s halftime, duel break 3 → 5 s, ostrzyżeni prep 8 → 10 s / break 5 → 7 s, result screen 15 → 20 s, bomb.result carries the break reason, plans in Polish. Wave 3, seven packages in parallel worktrees, each inspected by a code inspector and an eye inspector and audited: P1 CS2 death (killer card with HP and damage, death cam, spectating, late join, Polish error codes), P2 CS2 top strip (my side left, alive pips, freeze countdown in the clock, bomb icon + fuse after a plant, one mode line), P3 left column (radar with rim pins, money, Polish plan card, chat, hints), P4 corners (big numbers, icons for grenades/armour, kill feed), P5 moments (intro, round start, one round-end banner, halftime, match point, a moment bus: one banner and one alert at a time, beeps to the real release), P6 three-stage match result + CS2 Tab scoreboard, P7 transitions through black, loading card, ESC column with leave confirm, five-aisle shop. Medals: P-SRV, P0-A, P2; reprimands P1, P4, P5; demotions (replacement engineer) P3, P6, P7; final residual fixes b5af5dd, 21149bd, fbc841a. | apps/client/e2e/out/u/{before,p0-xhair,P1..P7}/states.md + PNGs (gitignored; regenerate with hud-states.mjs); docs/UI_U_SPEC.md | typecheck ✓ test ✓ (1219: shared 348, skins 11, server 222, client 638) build ✓ (per package) e2e ✓ 19/19 after P0+P-SRV; e2e after wave 3 — (final gate stopped by the owner; owner played it: "działa dobrze") | review |
...
| 2026-09-26 | W (DOLNA — the third map, owner's brief: a prompt first) | claude/confident-cray-nneqme (harness-assigned) | **"Przygotuj nową mapę pod turniej jak z CS:GO … na podstawie zdjęć … napisz prompt, wkleję go do innego czata" — the owner asked for the prompt, not the geometry, so the deliverable is `docs/MAP_3_DOLNA.md`: the block between its markers goes into a vision chat with the three photos, which writes the executive brief for the session that builds the map.** The document carries the engine facts transcribed from the code (player 0.70 × 1.80, mantle 1.25, sprint jump 4.42, the weapon ranges, the MapDef vocabulary, every generic-suite threshold, the tools and their traps) and was **fact-checked by nine verifiers: 325 claims, 22 corrected** (4.43 → 4.42; the floor-prop tolerance is 0.2 m; a flag zone is 1.5 × DOM.radius; only the moon casts a shadow — `LightHint.shadows` is unread; `map-audit.ts` and `map-review.html` fall back SILENTLY to the district on an unknown id; `duel-check.mjs`, cited by MAP_2.md, does not exist). Two dry runs (the receiving chat, the executive session) and a completeness critic reshaped it: the executive prompt no longer transcribes §4 (the session reads the file in the repo), the chat verifies the layout against the photos instead of inventing one, the drop letter is W (G is GÓRA), the map is registered in `MAPS` from the first geometry commit, and the work is two sessions with a sign-off between them. **The layout in §5 is measured, not drawn**: a harness (`apps/client/e2e/tools/dolna/measure.mts`) emulates `map.test.ts` + `map-duel.ts` on any prototype (validated on GÓRA to its published numbers), two rival designers produced measured prototypes before the session limit stopped the judges, and one synthesis pass merged them — 127 solids, starts 44.3 m apart and hidden (6.4 % / 4.1 % of surfaces see them), contested street points Δ ≤ 150 ms, a 58.6 m line on the road, nothing standable ≥ 1.9 m, nothing floating; the one red check, p90 28.3 m, is kept and named as D-W7 (the cover language leaves only the van and the toilet taller than a standing eye). Fourteen decisions D-W1…D-W14 and twelve owner questions P1–P12 with defaults are in the document. Game code untouched (the owner is editing locally). | `docs/MAP_3_DOLNA.md`; `apps/client/e2e/tools/dolna/` (harness, three prototypes, `measure-final.md`, `plan-final.txt`; rerun: `./apps/server/node_modules/.bin/tsx apps/client/e2e/tools/dolna/measure.mts apps/client/e2e/tools/dolna/proto-final.ts`) | typecheck ✓ test ✓ (1329: shared 373, skins 11, server 256, client 689) build — (docs and tools outside the bundle; not run) e2e — (not run) | review — waiting for the owner to paste the prompt and answer P1/P2 |
...
- 2026-09-23 — Drop T: **the profile is now written the first time the MENU is drawn, not the
  ...
  player who never presses GRAJ still accrues their one crate a day (which is what "one a day"
  says, and L5 still holds — nothing is bought), and `multiplayer.spec.ts`'s "a fresh browser
  profile" precondition had to stop asserting that no profile exists and start asserting that
  nothing has been EARNED — no xp, no matches, no badges — which is what it meant. (lead)
- 2026-09-23 — Drop T (owner): **the tournament bracket gets a new replicated field.** L6 makes a
  new schema field a stop-and-ask, and the owner was given three routes: `@type("string") bracket`
  on MatchState; riding the four `BombState` strings the duel never uses; or broadcasting the
  bracket on change. He chose the field. The two alternatives were not free — a field named after
  the bomb would be carrying a tournament bracket, and a broadcast leaves anybody who joins or
  reconnects mid-pair with no bracket at all until the next one ends. It is written only when the
  bracket changes, about 250 bytes for a draw of eight, and empty in every other mode. (owner)
- 2026-09-23 — Drop T (owner): **a tournament pair is played to `DUEL.wins` (6), not 3.** The owner
  was told what that costs — a draw of eight is seven pairs of 8–12 minutes, so over an hour, with
  six people watching for most of it — and chose the full match anyway. The number is one constant
  if an evening proves it wrong. (owner)
- 2026-09-23 — Drop T: **no fourth map; the 1 v 1 arena is GÓRA, pinned.** The owner asked for a
  new CS-style arena, and the code said something better: GÓRA already IS one (34 × 22 m, its
  symmetry a construction, its starts mutually invisible, 0 ms between the sides) and the mode
  simply never used it. A genuinely small new map would also have had to break the per-map test
  contract every map is held to — three buy stations spread over 30 m (`map.test.ts:115`), three
  Domination flags (`mapFlags.test.ts:16`), two bomb sites over 100 m² (`floorAudit.test.ts:29`) —
  which is a change to tests guarding every existing map, for one mode's benefit. (lead)
- 2026-09-23 — Drop P (owner's brief, "ulepsz perki, granaty wszystkie, i granatnik"): **the
  launcher's shell does 110, and the no-sliver rule now covers explosives.** `balance.test.ts` has
  held one line since drop B — a weapon whose perfect outcome lands in [95, 100) is an accident,
  because the shooter did everything right and the victim walks away — and it iterated `WEAPONS`,
  so the one weapon whose damage lives in `GRENADES` was never asked. The test now iterates the
  grenades and the shell as well. 110 is the smallest honest number: a direct hit kills, a heavy
  plate survives it on 45, and at 2 m the blast still does 70. (lead)
- 2026-09-23 — Drop P: **nothing explosive hurts its owner, and that stays true.** `applyDamage`
  refuses `v.id === attacker.id`, so your own frag, your own fire and your own shell are free, and
  in a team mode a team-mate's are too. CS charges you for all three and it is the single biggest
  difference left between this game's grenades and the ones they are modelled on. It was NOT
  changed here: every bot, the duel's economy and six modes' balance sit on top of it, the brief
  was to improve the grenades rather than to re-price the risk of using them, and an owner who
  wants a molotov that can kill the person who threw it should say so. Written down because the
  next agent WILL find it and think it is an oversight. (lead)
- 2026-09-23 — Drop P: **a smoke cloud stops a flashbang, and does not stop a frag.** Both were one
  `losBlocked` call before. The split is deliberate and it is the CS behaviour: the cloud is opaque
  to light and transparent to shrapnel, so a player who smokes a doorway is safe from the flash
  through it and still has to leave before the HE lands. (lead)
- 2026-09-23 — Release (owner): **the release number is 2.1** (`GAME_VERSION`, and 2.1.0 in all five
  package.json files); the beta label is off. And a note for the next agent who meets a failing
  tool: `reconnect.mjs` was written off twice as "a harness problem, it crashes on the untouched
  sources too". It crashed on the untouched sources because the bug was in the untouched sources —
  a one-second network blip ended the match while the SDK reconnected behind it. A tool that
  crashes where a player would be dumped to the menu is not a broken tool.
- 2026-09-23 — Release pass (owner's brief, "popraw całą grę bo ja wypuszczam"): **the two modes
  built on Counter-Strike now read one block of CS's rules, and Bomb moved to meet the 1 v 1.**
  Bomb's freeze went 30 s → 15 s and its shop stays open 5 s into the round; its kills pay CS's
  weapon table instead of a flat $300. That is a change to the mode's FEEL, decided under "zrób jak
  w CS:GO" and recorded here as such: matches are ~3 minutes shorter and the pistol round matters
  more. Nothing else about Bomb moved — the round is still 1:55, the fuse 40 s, first to 7 of 12.
  Two consequences of this session's own earlier work were treated as release bugs rather than
  Deferred notes: a crowd spawning inside itself, and TDM's kill limit ending a 40-body match in a
  minute. The e2e suite is green for the first time (18/18), so L7's merge bar is met. Decided by
  the agent under the brief; the owner's call still open on the version string (`GAME_VERSION` is
  "2.0 beta") and on the CS veteran's list in Deferred.
- 2026-09-22 — Duel / shop / settings (owner's brief): **the 1 v 1 is Counter-Strike's mode, with
  one deliberate departure, and the owner chose all three of the open questions.** Asked, he picked
  CS's economy *with a floor* ("klimat CS-a bez frustracji"), CS's own 15 s + 5 s buy window over a
  faster one, and Polish tabbed settings. The floor ($2,500 on every round but a half's first) is
  the only number here that is not CS's, and it is recorded as the owner's: it swallows the ladder's
  first two rungs, which a CS player will notice and which is the price of never playing three
  pistol rounds in a row in a one-on-one. Perks and the launcher are off this mode's shelf for the
  same reason the mode exists — a duel where one side regenerates 6 HP/s is not a duel — and that
  rule is now shared with Bomb instead of being a special case in the room. No replicated field, no
  tick or snapshot change (L6); nothing gated (L1). Decided by the agent under the brief, with the
  three choices above made by the owner in-session.
- 2026-09-22 — Seats (owner's brief, outside the lettered drops): **room capacity is a property of
  the MODE, and deathmatch's bots stop taking human seats.** The owner asked for "8 bots plus
  uncountable players" in deathmatch. "Uncountable" is not a number a server can hold, so the open
  cap is 32 humans by default (40 bodies with a full house of bots) with `FB_MAX_PLAYERS` to move
  it up to 64 — chosen from measurement (0.32 ms/tick, 3.6 kB/s per client at 40 bodies), not from
  taste, and documented as such in `constants.ts` and `docs/HOSTING.md`. TDM *and* FFA are open,
  because "deathmatch" names both and neither has a roster shape to protect; Bomb, Domination, The
  Boys, Gun Game and Ostrzyżeni keep twelve bodies, where the roster IS the mode. Nothing was
  gated (L1) and no wire field, tick or snapshot constant moved (L6). What this does NOT do:
  `MATCH.scoreLimit` is still 40 kills whatever the roster, so a full open lobby will end on the
  kill limit in a fraction of the seven minutes — see Deferred; scaling it needs the owner's word
  because it changes how every TDM match ends. Decided by the agent under the brief; the cap and
  the score limit are the owner's to settle after a playtest.
- 2026-09-14 — Polish pass (owner's brief, outside the lettered drops): **a match draws the
  procedural body and the procedural guns, so the animation work went into `Character.ts`, not the
  imported rig.** `Game.ts` empties the manifest's characters and weapons before assets are prepared
  (the Drop A proposal of 2026-09-07 still stands). Three presentation changes with a fairness angle
  were made and are recorded here: the reload animation now reflects the real magazine (tactical
  reload skips the charging handle, N shells load N shells — the server's `reloadMs` is unchanged, so
  nothing about timing or ammo moves); a long gun is held in a bladed stance (torso turned toward the
  gun, gun turned back, bore still on the aim — `hand-pose.mjs` 3.5–3.9°) so the arm IK can put both
  hands on it, which widens the drawn silhouette of a long arm by up to 25 mm (hitbox untouched); and
  the viewmodel's kick is sized from `recoilUp` rather than a heavy/light bucket, with the VZ-9 pinned
  to its shipped constants. No wire field, tick, snapshot rate or interpolation constant changed (L6).
  Decided by the agent under the brief.
- 2026-09-13 — Drop G, tournament pass: **GÓRA is the roof, not the flat, and it is a 180° rotation
  of itself.** The owner's brief asked for a 1v1 arena that is "not necessarily mirrored but with no
  advantage to either side"; a rotation gives both players the same map from the same hand (a mirror
  swaps left and right), and it makes fairness something the test checks on the geometry rather
  than something tuned. The `duel` mode reuses Bomb's and Ostrzyżeni's round machine and replicated
  fields (L6, no new wire field). The cage is 4.4 m because the climb test found every lower height
  reachable by a chain of real jumps; if the owner wants a lower wire, the structures near the
  parapet have to come down with it. Decided by the agent under the brief; the owner's call on the
  lip height (1.35 m) and the 60 s round clock after a human playtest.
...
- 2026-09-09 — Drop K: the parody outfit is named **NIETOPERZ Z MAREK**, not the licensed name the
  ...
  one string. (lead)
- 2026-09-10 — Drop L (proposed, owner to confirm): the district's **identity is the long lane** and the
  rework must not quietly remove it. `docs/MAP_2.md` §1–2 measured its median clear sight line at 24.0 m,
  36.6 % of clear lines over 30 m, longest 111 m — and GÓRA exists precisely to be the 5 m map. So the
  brief tells the next agent that closing a lane, moving a spawn or shifting that distribution is a STOP
  AND ASK, and that the art pass is judged on readability and vibe rather than on layout. If the owner
  wants the district's fights shortened as well as prettified, that is a different and larger job and it
  should be said before the geometry moves. (lead)
- 2026-09-25 — Drop U (owner's brief, Ultron): round breaks lengthened (bomb 7 s + 15 s halftime, duel 5 s, ostrzyżeni prep 10 s / break 7 s, result 20 s) — CS2's rhythm, the owner's "mało czasu na wszystko". `bomb.result` now also carries the break reason in ostrzyżeni and is cleared at every duel freeze: a behaviour change to an existing replicated field, no schema change (spec §9 Q2).
...
- Drop T 2026-09-23: **a waiting entrant has no camera.** In a tournament six of eight people are
  dead for minutes at a time, and nothing moves their view: they get the bracket between pairs and
...
- Drop T 2026-09-23: **shortening the duel's approach needs the stair fixed first.** Measured:
  ...
  the steps square) and the start can come 5 m closer, which is worth about a second and a half of
  every round's opening.
- Drop T 2026-09-23: a tournament has no third-place match, no re-seeding when somebody withdraws
  before the draw, and nothing survives the room — all deliberate, and all cheap to add if an
  evening asks for them.
- Drop P 2026-09-23: **the launcher's shell has no arming distance and no spread.** Fired into a
  wall at arm's length it puts a 110-damage blast on your own position, which costs you nothing
  (see the self-damage decision above), so the most expensive gun in the shop is also a free
  panic button against somebody standing on top of you; and the shell leaves the muzzle on the aim
  line exactly, so it is pinpoint at any range. Both were designed this drop and both were killed
  in review: an arming distance and a cone change how the weapon PLAYS, and the brief was to make
  it read honestly, which it now does. They are the two changes to make if the owner ever says the
  launcher is too strong.
- Drop P 2026-09-23: **`burnAcc` is never cleared when a player walks out of a fire.** Fire is
  billed in whole points every time the accumulator passes 5, so a player who dips in and out of
  three pools carries the remainders from the first two into the third and takes a point sooner
  than they should. It is at most 4 HP over a life, it is reset on every spawn, and clearing it on
  exit would make brief contact with fire completely free — which is a balance change, not a fix.
- Drop P 2026-09-23: two things the evidence tool could not do in this container, for whoever
  writes the next one. **A lone client cannot be hurt** (the server refuses damage from a player to
  themselves), so the steroids' gate is measured by stamping the store's `damageAt` — the same
  field a `Damaged` packet writes — and a measurement of the gate against a real bullet needs a
  second client or a bot. And **driving the trigger through `inputState` races the render loop**:
  SwiftShader gives a few frames a second here, a semi-automatic weapon takes a rising edge only,
  and a button pressed and released between two frames is never seen. `grenade-feel.mjs` counts
  frames (`requestAnimationFrame`) rather than milliseconds and retries the shot; the first draft
  of it reported a launcher that would not fire and a frag that never left the hand, and both were
  the harness.
- ~~Release 2026-09-23: `GAME_VERSION` still reads "2.0 beta"~~ — **done the same day**: the owner
  set 2.1.
- ~~Release 2026-09-23: `reconnect.mjs` still crashes~~ — **done the same day, and it was not the
  tool**: see the R2 row. What remains of it is that reconnection has no automated regression test
  beyond `errorEndsMatch`'s unit test and the tool itself; a Playwright spec that drops a context
  offline would close that.

- **Duel 2026-09-22, the CS veteran's list** (playtest persona, `apps/client/e2e/out/duel/duel-check.md`).
  Each is a real difference from CS and each is the owner's call, not an agent's: (1) the clock
  awards a round to whoever has more health, which CS never does — 30 damage and then 55 s of hiding
  is a free round; making it a draw risks a match of draws, so it wants a decision, not a patch.
  (2) No helmet: armour is a 50/100-point plate, so the $650-vs-$1,000 pistol-round decision every CS
  player makes does not exist here. (3) Selling refunds 70 %; CS refunds 100 % inside the buy window.
  (4) No walk modifier at all — Shift is sprint, and there is no silent movement. (5) No buy binds,
  no rebuy, no autobuy.
- **Duel 2026-09-22: 47 % of a match is spent frozen or waiting** (the impatient persona's
  arithmetic: 15 s freeze + 3 s break per round, 162 s of a ~342 s six-round match). CS's numbers,
  chosen by the owner with that cost named. If it turns out to drag, the cheapest fix is releasing
  the freeze early once both players have shut the shop — which CS does not do either.
- **Duel 2026-09-22: a tie at the 15-minute cap plays on** (`DUEL.matchMs` only ends a match
  somebody leads). By design since drop G, but nobody has hit it; a decider round would end it.
- **2026-09-22: no bot fills an empty duel seat while you wait**, and the wait has no ETA — the
  persona's alt-tab-and-never-return moment. A bot after ~20 s of waiting would cover it.

- **Seats 2026-09-22: TDM's kill limit does not scale with the roster.** `MATCH.scoreLimit` is 40
  kills, which was a seven-minute match at 6 v 6 and is about a minute at 20 v 20. FFA's 30 is the
  same story. A limit derived from the body count would need either a replicated field (schema
  change — ask first) or a pure function both ends compute from the replicated player map; the
  second is cheap and is what I would do. Owner's call, ideally after one crowded playtest.
- **Seats 2026-09-22: nobody has drawn forty characters.** The server serves the cap comfortably;
  the CLIENT's cost at 40 bodies (draw calls, the procedural character's LOD, nameplates) is
  unmeasured — every render figure in this repo comes from rooms of twelve. `profile.mjs` with a
  crowded room is the measurement, and it wants a real GPU.
- **Seats 2026-09-22: spawns were designed for sixteen points and twelve people.** `pickSpawn`
  penalises a point within 3 m of a living ally, but with 20 a side there is no point that is not
  near somebody; a crowded respawn wave releases everyone at once. Expect stacking at the wave
  release. A per-team spawn ring, or waves released in two halves, would be the fix if a playtest
  says it matters.
...
- Drop L: `apps/client/e2e/tools/map-review.mjs` launches Chromium with `channel: 'chrome'`, so it needs a
  real Chrome install and cannot run where the sandbox ships only a pinned Chromium. Every other tool in
  the repo honours `PW_CHROMIUM` (`playwright.config.ts`, `body-shots.mjs`) and/or `PW_CHANNEL`.
  It is the ONE tool that renders the map, so the map's own review surface is the one that will not start.
  Left alone here because this session wrote no code; `docs/MAP_1_REWORK.md` §9 asks the next agent to fix
  it to the repo convention rather than work around it.
- Drop L: nobody has RENDERED NIGHT_DISTRICT in this session either — every number in the brief is map DATA
  measured through `tsx`. The defects it names (lights inside posts, unlit bands, colour soup) are real in
  the data; what the map actually LOOKS like, and therefore which of them matter most, still needs the six
  `map-review.mjs` views and the owner's eyes.
- Drop L: `floorAudit.ts` audits **top faces only**. Measured on NIGHT_DISTRICT: it reports 0 pairs, while a
  six-direction check finds **54** same-direction coplanar faces at exactly 0.0 mm on interpenetrating solids,
  44 of them over 1 m² — walls, ceilings, staircases and every building corner built by `districtExpansion`'s
  `room()` helper. The audit's own docblock argues the case for top faces (a floor a player stands on); the
  same argument holds for a wall a player looks at. Extending it is a small change to a pure module plus a
  gate, and `docs/MAP_1_REWORK.md` §14.4 asks the next agent to do it rather than nudge 54 boxes.
- Drop L: **ten of the eighteen lamp props have no collision solid at all** while six identical-looking street
  lamps do (`lamp_post_*`). Nothing tests that a prop which reads as cover IS cover, and on a tournament map
  that is a fairness bug rather than a cosmetic one. Named in the brief; a general test ("every prop kind that
  a player would take cover behind has a solid") would be the durable fix and belongs to whoever does Drop L.
- Drop L: the **6v6 tournament work is not done** and it is the half that decides a prize. Measured,
  unchanged: TAPER's median walked path is **10.7 m** shorter to bomb A and **17.4 m** shorter to bomb B
  (both sites on the same side, no compensating pair) while FADE is 17.6 m closer to RECEPTION and 11.8 m
  to BOOTH; and each side's eight spawns span ~55 m of x inside ~3 m of z, so a six-stack starts strung
  across the whole map and cannot leave spawn as a unit. `docs/MAP_1_REWORK.md` §15 has the table;
  `map-audit.ts` §7 re-measures it. This is layout work and wants the owner's word on whether the two
  sites move or the format swaps sides.
- Drop L: **ten of the eighteen lamp props still have no collision solid** while six identical-looking
  street lamps do, so the same lamp stops a bullet on Main Street and not in the yard. `map-audit.ts` §5
  counts them. Unfixed here because the choice — every standing lamp becomes cover, or none do and the
  drawn post gets thin enough to read as decoration — changes cover on a tournament map either way.
- Drop L: the lone `wheel` prop at (12.5, 0, 20) is still a single tyre standing in an open yard.
- Drop L: Codex replaced the range-based light exclusion with a per-mesh `selectPracticals` choice, which
  is the right fix for the lighting POP recorded above — but it leaves every one of the 53 practicals
  holding an `excludedMeshes` array of nearly every mesh, and Babylon's `canAffectMesh` does a linear
  `indexOf` over that array per light per mesh per frame. It is plausibly a real per-frame cost and
  nothing in this session measured it: it needs `map-review.mjs` on a real GPU, not a data audit. The
  cheaper primitive is `includedOnlyMeshes` (short lists), which Codex rejected in a comment because it
  would leave characters built later unlit — true, and answerable by including dynamic meshes as they
  are created.
- Drop L: the `corrugated_* → paint_*` remap in `addToZone` is justified in its comment as "the ribs are
  already geometry". They are not, for the four containers: `dressing.ts`'s container case draws one
  plain box plus corner posts and rails, no ribs. The decision may still be right for the flat-colour
  direction, but the stated reason is wrong, and MEASURED it saves **0** merge groups on the structural
  pass (193 either way) — whatever it saves is in material and texture count, or on dressed objects.
- Drop L: **Domination, TDM and Ostrzyżeni still spawn on `p.team` and never swap**, so the district's
  asymmetry IS a standing advantage there: measured after the spawn rework, team 1 reaches flag A 16.2 m
  and flag C 19.7 m sooner, team 0 reaches flag B 13.6 m sooner — net, team 1. Bomb is fair because
  `bombSpawnSide` follows the role; the same trick at a mode's midpoint would fix the others, but it is a
  change to modes the tournament brief did not name and it wants the owner's word first.
- Drop L: the audit's cover check counts prop KINDS `crate`, `trash` and `pole` as cover, and twelve of
  those (bin bags, small crates, the barber pole, a sign pole) have no solid. Unlike the lamps that is
  probably right — they are dressing, not cover — but the audit cannot tell the two apart and says so.
- Drop L: **the map is now lit right and still EMPTY**, which is the owner's other complaint and is
  content, not light. The west yard is 221 m² of bare concrete with no prop on it and it reads exactly
  that way in `e2e/out/map1/after2/west_yard.png`; the two extension blocks are 2 546 m² at a third of
  the interior's density. This is the next slice and `/viewer` is how it should be judged.
- Drop L: three things the FIRST viewer session made visible that no data audit could — at site A the
  "ZAJEZDNIA / SERWIS" sign hangs in mid-air at an angle with its text running off the plate, a red
  panel floats beside it, and a lamp head sits on a post at a height that reads as a lollipop. Props
  are visual-only and the map tests only check that a wall prop touches a wall, so nothing catches a
  prop that touches one at the wrong angle. Screenshot: `e2e/out/map1/viewer/site-a.png`.
- Drop L: the viewer renders at 2-3 fps under SwiftShader in this container, which says nothing about
  a real machine and is why the panel shows draw calls and mesh counts as well as fps — those two are
  hardware-independent and are what a report from the viewer should quote.
- Polish pass 2026-09-14: `hand-pose.mjs` reports the **clippers** bore 15° off facing on every bot —
  it is the melee hold's own 0.25 rad nose-down (`Character.update`, `+0.25 * melee`), pre-existing and
  by design, but the tool's 5° acceptance line counts it as a failure every run. Either exempt slot 3
  in the tool or give the clippers their own oracle.
- Polish pass 2026-09-14: `hand-pose.mjs` once reported `lmg bot-0 meshes 8/0` (the gun built but not
  yet drawn when measured); a one-frame race under SwiftShader, not reproduced. The tool could wait a
  rendered frame after the weapon swap before reading.
- Polish pass 2026-09-14: `profile.mjs` still teleports to Night District's ORIGINAL coordinates
  (street/shop/hall/yard/storage) and hard-codes the `chromium-1194` path, as does `hand-pose.mjs`;
  both run with `PW_CHROMIUM=/opt/pw-browsers/chromium` and the views are whatever the teleport lands
  in. Re-point the views at the rebuilt district's areas.
- Polish pass 2026-09-14: the support hand of a third-person long gun still sits 2–4 cm off the
  fore-end line at the steepest downward aim (pitch 0.9) on the SMG and the shotgun family: the arm
  cannot reach further with the gun where the vest clearance needs it. A shoulder that drops with the
  aim, or a slightly shorter TP hold on those two, would close it.
- Polish pass 2026-09-14: the first-person light rig and the night cube were judged on SwiftShader
  screenshots only. On a real GPU the key at 0.55 / rim at 0.35 / environment 0.7 may want a nudge;
  the numbers are in `Viewmodel.buildRig` and `createWeaponMaterials`.
- Polish pass 2026-09-14: `reconnect.mjs` crashed on both the before and the after sources in this
  container — see the ledger row of the day for what it printed; it is a harness or a reconnect
  regression older than this pass, and it needs a look before the next multiplayer session.
- Polish pass 2026-09-14, pre-existing and unchanged by it (measured on the untouched sources in the
  same container): `vm-fit.mjs` counts near-plane cuts in ADS on the **carbine (104 verts) and the
  shotgun (20)** — a stock through the near plane at the ADS distance; the Playwright specs "armoury
  remembers an equipped finish" (the locked `slickback` haircut button is not disabled) and "gun game:
  the whole ladder to the end" (ladder stays at 1/14) fail before and after.
- Drop U: the final gate (full 69-scenario gallery with --budget at 3 sizes, e2e after wave 3 incl. new hudfeed.spec and startup fade tests, veil-cost on a real GPU) was not completed — the owner stopped it after playing the build. Run it before merging to main. Spec §9 questions Q1 (MVP stars need a field), Q3 (Polish mode names in the menu), Q4 (THE BOYS class names) are open. P1 spectates after the 1 s death beat in a round break instead of +5300 ms (the duel's 5 s break would never reach it).
