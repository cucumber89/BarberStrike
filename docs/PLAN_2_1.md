...
| 2026-09-21 | D (fix, owner's report) | claude/upbeat-ramanujan-cmvme6 (harness-assigned) | **"Nie można grać online — jak ktoś tworzy mecz, inni go nie widzą."** Measured first, code second. Node against the SDK: a created room IS listed by `/rooms` and `/health` and a second client can `joinById` — so not Colyseus 0.18's `query`, not the `private`/`locked` filter, not a second process (compose runs ONE `game` container behind Caddy). Then two real browsers against one process serving the built client (`lobby-visibility.mjs`): a plain QUICK PLAY room shows on the guest's title screen and lobby as `1/12 JOIN` — fine. A room CREATED WITH BOTS is where it broke: `publishCount` put `state.players.size` (bots included) into `metadata.players` while `slots` was already the HUMAN seats (`12 − bots`), so a host alone with six bots was published as **7/6** — the lobby drew it as **FULL**, greyed out JOIN and sorted it last, `/health` said 7 playing, and nobody could get into a match with five free seats; with 8+ bots every such room was unjoinable from the moment the host was in. `onJoin` already subtracted the bots (so a link join worked — only the browser lied). Fix: one `humanCount()` used by `onJoin`, `publishCount`, and therefore `/rooms` and `/health`; browser now reads `1/6 JOIN` on the same room. Regression test in `Spectator.test.ts` (six bots, one human → `players` 1 < `slots` 6, second human joins → 2). The live server could not be reached from this session (proxy 403), so whether barberstrike.click runs the current image is unverified — check with `curl https://barberstrike.click/rooms` while a match is open. | `apps/client/e2e/out/d/lobby-visibility.md` (9/9; regenerate with `apps/client/e2e/tools/lobby-visibility.mjs`) | typecheck ✓ test ✓ (shared 306, server 191) build ✓ e2e — | review |
