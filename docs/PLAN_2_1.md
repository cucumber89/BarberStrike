...
| 2026-09-09 | J | claude/eager-volta-0wc4f5 | Slice 2, four player-facing asks from the owner. **Crouch is on Ctrl again.** It was moved to C because crouch-walking forward on a Ctrl bind IS Ctrl+W and no page can refuse Ctrl+W — true then and true now — but a lot of people crouch on Ctrl and moving the default did not change that, it just cost them their crouch. So the danger is MITIGATED rather than avoided: `LOCKED_CODES` now asks Keyboard Lock for the union of `RESERVED_BY_BROWSER` and `INTERCEPTABLE_CHORDS`, so a fullscreen Chromium player never depends on interception timing at all; and `unloadGuard.ts` arms `beforeunload` for the life of a match so every other browser ASKS before closing the tab. Both keys ship (`["KeyC","ControlLeft","ControlRight"]`), the controls screen still says what Ctrl costs, and the settings migration that used to strip a saved Ctrl bind is deleted. Three tests in `browserKeys.test.ts` asserted the old contract and now assert the mitigation, with the reversal written into their docblock so it reads as a decision rather than a drift. **The slide is back**, restored from commit b0ac372 (deleted by PR #14) with its numbers unchanged so it feels like the move people remember, and its five original tests pass unmodified. What is NOT restored is the pair of REPLICATED fields it used to carry: the plan gates schema fields, and they turn out to be unnecessary — both ends derive the slide from the same buttons with the same rule, and an observer reconstructs it with `slideSeenUntil`. That took a measurement to get right: the obvious "crouching and faster than a crouch walk" test is FALSE, because the fastest possible crouch walk (Scout 1.15 x energy drink 1.08 x mobility 1.06 = **3.555 m/s**) is above the 3.2 m/s this slide ends at, so a Scout on an energy drink would have been drawn sliding whenever they crouch-walked. The rule arms on the entry burst (4.5 m/s, unreachable by any crouch walk, held for the first ~340 ms of every slide) and then runs the slide's own 800 ms. Four more tests cover that, including the fastest crouch walk in the game in six directions. **Assists in the kill feed**: "PLAYER + PLAYER" before the weapon. The server always knew who assisted — it pays them — but it worked that out AFTER broadcasting `S2C.Kill`, so the names could not be on the message; the loop moved above the broadcast and `assists` rides as a MESSAGE field, which costs nothing in the snapshot budget and adds nothing to `PlayerState`. **A headshot icon**: an arrow striking a head in profile, inline SVG in the style of the shave's razor, replacing a `✦` appended to the weapon's name that said "headshot" only to someone who already knew. Written up here because the e2e that proves the slide also proves the Ctrl bind (it presses ControlLeft) and the watcher's reconstruction (it reads `RemotePlayer.sliding`, not a schema field). Found on the way, in the harness rather than the game: `faceOff` hands every caller the SAME spawn pair, so a bystander standing on the next shooter's mark eats the bullet as a team-mate — no damage, no error, a healthy-looking Shot broadcast, and half an hour spent looking at the gun. `KillFeed.test.ts` carries a `clearRange` helper and that explanation. | `apps/client/e2e/out/ui/{hud-killfeed.png,killfeed-zoom.png}` — the real `Hud` with one row of every shape it can draw (plain, headshot, assisted, assisted headshot, shave); regenerate with `node apps/client/e2e/tools/hud-shot.mjs --url <dev> --label killfeed` | typecheck ✓ test ✓ (762: shared 247, skins 2, server 178, client 335) build ✓ check:weapons ✓ (19/19) **e2e ✓ 16/16** (8.0 min, idle machine) including a restored two-client slide test that asserts the server agrees (≤ 2 corrections). Three e2e tests were flaky for reasons that were not their subject — the smoke opacity's exact 1, a mark clicked at a corpse, and my own slide test starting on a 14 m lane a 13 fps client cannot use — all diagnosed and fixed as tests; see the commit and Deferred. | review |

| 2026-09-09 | J | claude/eager-volta-0wc4f5 | Slice 3, one line long and entirely about git. The owner played the game after slice 2 was pushed and reported seeing **none of the four changes**, suspecting the skins work added meanwhile had broken them. It had not, and nothing in the game was wrong: **PR #26 was merged at head `7bb177e`, which is slice 1's last commit**, and slice 2 was pushed to the same branch afterwards — so the four asks existed only on the branch and were in no build anybody could play. `origin/main` at `0ec8a13` states it exactly: `crouch: ["KeyC"]`, zero mentions of the slide in `movement.ts`, no `assists` on `KillEvent` — while `inputDt.ts` IS there. That asymmetry is the whole answer and it is worth keeping: the smoothness work the owner asked for FIRST is in the game they played, and only the four later asks are missing, which is why "no changes" was a report about the second brief and not a regression in the first. The skins are innocent on the evidence, not on assurance: PR #27 touches `Armoury.tsx`, `Menu.tsx`, `menu.css`, `menuArt.tsx`, `progression/profile.ts` and its two tests — **disjoint from all 25 files of slice 2 except this document**, and replaying slice 2 onto `0ec8a13` produced zero conflicts. Per the branch rule for an already-merged PR, the branch was RESTARTED from `origin/main` rather than stacking on merged history: `git rebase --onto origin/main 7bb177e`, so the three commits now sit on `0ec8a13` and the branch is `main` plus slice 2 and nothing else. Verified with the armoury actually present, which is the only claim worth making here — the two changes coexist, they were never in conflict, and the reason to be sure is a green suite over both rather than a clean-looking diff. | none new; the diagnosis is `git log --oneline origin/main..HEAD` against PR #26's merge head `7bb177e`, and `git show origin/main:<file>` for the three absent features | typecheck ✓ test ✓ (763: shared 247, skins 2, server 178, client 336 — the +1 over slice 2 is the armoury's own profile test) build ✓ check:weapons ✓ (19/19) e2e — **outstanding at the time of this row**, 17 tests collected (the armoury's spec now sits beside the slide's); the first run was lost to my own tooling rather than to a failure — the command was piped through `tail`, which buffers, so when the process ended there was no output at all to read. Re-running; the result gets its own row rather than an edit to this one | review — needs a NEW PR; #26 is merged and cannot carry these three commits |

| 2026-09-09 | J | claude/eager-volta-0wc4f5 (PR #28) | Slice 3's outstanding e2e, and the row exists because getting to the number took three runs and two of my own mistakes. **The result: 17/17 in 9.0 min, on freshly restarted servers with nothing else on the machine.** Both halves of the coexistence claim are in it — `armoury.spec.ts` (4.2 s) and the slide test that also presses ControlLeft and reads the watcher's reconstruction (29.2 s). The run before it reported **2 failed / 15 passed in 15.5 min** and that result is worthless: I had TWO Playwright suites live against one Colyseus server, both driving `dev:teleport` and `dev:endmatch` in the same room. So the failures were not called flaky, they were **retired by evidence** — the two that failed (`gun game: the whole ladder to the end`, and `clean entry, deferred spawn, visible buy timer and shop categories`) both pass in the clean run, and 15.5 min against a normal 8–9 is the contention showing up in the wall clock. This is the documented condition the collision bench already caught me on once, which is the argument for making it a rule rather than a memory: **one suite at a time, and read the run time before reading the failures.** Two tooling faults worth writing down because both cost a whole run. Piping the suite through `tail` buffers everything, so a run in progress is indistinguishable from a dead one and a killed run leaves nothing at all to read — redirect to a log instead. And `pgrep -c playwright` matches process NAMES, so it answers 0 for a live run named `npm exec playwright test`; worse, `pgrep -f <pattern>` then matches this harness's own wrapper shell, whose command line contains the pattern, so it never reaches 0 and cleanup looks stuck. `ps -eo args --no-headers \| grep -v grep` is the check that tells the truth. On the strength of the first bad result I nearly reported two failures as real. | `apps/client/e2e/out/**` unchanged; the run itself is the evidence, reproduced with fresh `pnpm dev` (`FB_DEV_TOOLS=1`) then `npx playwright test` alone | **e2e ✓ 17/17 (9.0 min)** — supersedes the outstanding field on the row above; typecheck / test (763) / build / check:weapons unchanged from that row | review |
