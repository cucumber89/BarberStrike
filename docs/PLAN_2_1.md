...
- **Release 2026-09-23: `GAME_VERSION` still reads "2.0 beta"** (`packages/shared/src/constants.ts`,
  printed in the menu, the pause card and `/health`; `package.json` says 2.0.0-beta). If this build
  is the release, that string and the package version are the owner's to set — an agent guessing a
  release number is not a decision it gets to make.
- **Release 2026-09-23: `reconnect.mjs` still crashes** (noted first on 2026-09-14 and unchanged;
  it crashed on the untouched sources too, so it is the tool or an older regression, not this pass).
  It is the one multiplayer path the suite does not cover end to end.
