...
| 2026-09-14 | M (new: gameplay polish, owner's brief `FABLE_GAMEPLAY_POLISH_BRIEF.md`) | claude/busy-bardeen-wqmxtl (harness-assigned; stands in for `drop/m-polish`) | **Shop, results, first-match path, measurements — four stage commits.** SHOP: four aisle tabs on a rail, one aisle at a time, ten rows max (the digits' reach); names 17 / roles 14 px, large silhouettes (per-icon viewBox from measured bboxes), price separated from the swap settlement (dopłata / zwrot), reasons as words with the amount ("Brakuje $1,400" from shared `buyShortfall`), state words on every button, clicks greyed until the server answers, `ShopResult.sold` for the sell line, ONE catalogue (`shopCatalog.ts`) for the printed shortcut and the handler — the tenth primary printed `1·10` and was unreachable, and key auto-repeat on an aisle digit bought item 1; both fixed and tested. `ui-fit.html` now has the game's cascade (fonts + cinematic.css); `ui-fit.mjs` measures every tab at 8 sizes with floors 16 / 13 / 10 px and no truncation — old tool: 9 px floor, 125 % sizes FAIL with five buy buttons off screen; new: 22/22 PASS across tdm / bomb / boys. RESULTS: `MatchResult` (verdict from own seat, score, one derived "why", three mode-fitted stats, XP bar + headline reward, details under a disclosure, table under a Tab tab, fixed footer that says the server restarts by itself, LEAVE), `RoundBreak` (bomb result string + attacking side, duel reason + Prep winner kept as new `hud.roundWinner` because the 10 Hz sync overwrote `winner`, Ostrzyżeni winner), competing overlays hidden when ended, reduced motion honoured; `resultText.ts` pure and tested against every server ending; seven result cases × two sizes measured. FIRST MATCH: `MODES[mode].objective` (one sentence, goal + win condition) on loading / warm-up / countdown / pause; Polish on every changed screen; hint no longer covers the buy prompt; nearest crate challenge on the result card; five tester tasks in PLAYTEST_TEMPLATE.md (no testers this session). MEASURED (all SwiftShader): 10 menu→match→leave cycles flat (977 / 121 / 44 / 14 / 31), heap 80.7 → 81.7 MB, cold READY 6.6 s vs warm 4.1–4.8 s; forced WebGL context loss recovers; Low = same draw calls as High (352 vs 351), only shadows and 14 shaders fewer; maxLightsPerMesh 57 everywhere; hud-bench before/after within noise (0.54→0.50, 0.93→0.73 ms/frame, commits/frame identical) — no HUD refactor claimed. Stale tests fixed rather than skipped: gun game spec had an 11-rung ladder copy (now `GUN_GAME.ladder`), armoury spec expected a locked haircut disabled (it is a preview button since 09-13), three tools had a dead Chromium path and one a dead scratch path. NOT done, written to Deferred: real-GPU fps, input-latency × RTT matrix, Low draw-call budget, per-light mesh lists, Boys role names in Polish. | docs/GAMEPLAY_POLISH_2026-09-14.md (report); apps/client/e2e/out/ui/{shop-fit-*.md,shop-*.png,result-*.png,round-end.png}, out/before/ (old shop), out/perf/{cycles-before.md,context-loss.md,profile-presets.txt}, out/hud/hud-*-{before,after}.json, out/m/{round-end-bomb.png,shop-ingame-after.png}; regenerate: `node apps/client/e2e/tools/ui-fit.mjs [--mode boys\|bomb]`, `cycles.mjs --n 10`, `context-loss.mjs`, `PRESET=low pnpm profile`, `hud-bench.mjs --drive all` | typecheck ✓ test ✓ (919: shared 306, skins 11, server 190, client 412) build ✓ check:weapons ✓ e2e 17/18 in one full run of 11.4 min — every multiplayer and startup spec green; `armoury.spec.ts` fails on the crate reel's marker alignment (63.8 px off), pre-existing and untouched by this drop (Crates.tsx not modified), two of its earlier stale assertions fixed; see Deferred | review |

## Decisions log (append-only)
- 2026-09-14 — Drop M (owner's gameplay-polish brief, harness branch `claude/busy-bardeen-wqmxtl`
  standing in for `drop/m-polish`): **the buy menu is four aisle tabs, not one screen.** The
  one-screen rule (Shop.tsx, 2026-09) bought "everything visible" with 9.5 px stat lines, names cut to
  "M-1 Clean Li…" and, at 125 % zoom, five buy buttons below the fold — measured by the old
  `ui-fit.mjs` as FAIL on two of its own sizes. Ten rows at most per tab is also exactly the reach of
  the digit keys, so the tab IS the first key of the two-key shortcut. The floors the tool now
  enforces (names 16, roles 13, all 10 px, no truncation) are the design target the brief names, and
  they are the rule for any future shop change: adjust the layout, never the type. (agent, on the
  brief's explicit allowance; owner to confirm by playing.)
- 2026-09-14 — Drop M: **`ShopResult.sold` added** (optional, message-level, not schema) so the
  client's result line can say "Sprzedano" without guessing from its own last request. L6 untouched:
  no schema field, no tick or snapshot change.
